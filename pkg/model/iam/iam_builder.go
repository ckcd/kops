...
func (b *PolicyBuilder) BuildAWSPolicyMaster() (*Policy, error) {
	...
	if b.HostedZoneID != "" {
		addRoute53Permissions(p, b.HostedZoneID, b.IAMPrefix())
	}
...
func addRoute53Permissions(p *Policy, hostedZoneID string, iamPrefix string) {
	...
	p.Statement = append(p.Statement, &Statement{
		...
		Resource: stringorslice.Slice([]string{iamPrefix + ":route53:::hostedzone/" + hostedZoneID}),
	})

	p.Statement = append(p.Statement, &Statement{
		Effect:   StatementEffectAllow,
		Action:   stringorslice.Slice([]string{"route53:GetChange"}),
		Resource: stringorslice.Slice([]string{iamPrefix + ":route53:::change/*"}),
	})
